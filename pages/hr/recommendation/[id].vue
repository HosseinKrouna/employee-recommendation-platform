          ...
          <div>
            ...
            <div class="flex items-center gap-3 text-lg text-muted">
              ...
              <span class="flex items-center">
                ...
                {{ recommendation.department || 'Keine Abteilung' }}
              </span>
            </div>
          </div>
          <div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div class="card">
            ...
            <div v-if="recommendation.cvFilePath">
              <button
                @click="downloadCV"
                :disabled="downloadingCV"
                class="btn-primary inline-flex items-center"
              >
                <svg v-if="!downloadingCV" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
                <span v-if="downloadingCV">Lädt...</span>
                <span v-else>CV herunterladen</span>
              </button>
            </div>
            <div v-else>
              <p class="text-muted">Kein CV hochgeladen</p>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-6">
          <!-- Recommended By Card -->
          <div class="card">
            <div class="bg-gradient-to-r from-green-600 to-teal-600 px-6 py-4 -mx-6 -mt-6 mb-6 rounded-t-xl">
              <h2 class="heading-2 mb-0 flex items-center text-white">
                <svg class="h-6 w-6 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="flex flex-col items-center text-center">
              ...
              <h3 class="mt-4 text-lg font-bold text-primary">
                {{ recommendation.recommendedByUser.firstName }} {{ recommendation.recommendedByUser.lastName }}
              </h3>
              <p class="text-sm text-muted mt-1">{{ recommendation.recommendedByUser.department || 'Keine Abteilung' }}</p>
          ...
          <div class="card">
            ...
            <div class="space-y-3">
              ...
              >
                <span class="flex items-center">
                  <span :class="getStatusDotClass(status.value)" class="mr-3"></span>
                  {{ status.label }}
                </span>
...
const getStatusBadgeClass = (status: string) => {
  const classes: Record<string, string> = {
    SUBMITTED: 'badge badge-status-submitted inline-flex items-center px-6 py-3 text-base',
    IN_REVIEW: 'badge badge-status-review inline-flex items-center px-6 py-3 text-base',
    APPROVED: 'badge badge-status-approved inline-flex items-center px-6 py-3 text-base',
    REJECTED: 'badge badge-status-rejected inline-flex items-center px-6 py-3 text-base'
  }
  return classes[status] || 'badge inline-flex items-center px-6 py-3 text-base'
}

const getStatusDotClass = (status: string) => {
  const colors: Record<string, string> = {
    SUBMITTED: 'h-2.5 w-2.5 rounded-full bg-blue-400 animate-pulse',
    IN_REVIEW: 'h-2.5 w-2.5 rounded-full bg-yellow-400 animate-pulse',
    APPROVED: 'h-2.5 w-2.5 rounded-full bg-green-400 animate-pulse',
    REJECTED: 'h-2.5 w-2.5 rounded-full bg-red-400 animate-pulse'
  }
  return colors[status] || 'h-2.5 w-2.5 rounded-full bg-gray-400'
}
...
const updateStatus = async (newStatus: string) => {
  if (!confirm(`Status wirklich auf "${getStatusText(newStatus)}" ändern?`)) {
    return
  }
...
const downloadCV = async () => {
  ...
  try {
    const token = localStorage.getItem('token')
    const response = await fetch(`/api/recommendations/${route.params.id}/cv`, {
      headers: {
        'Authorization': `Bearer ${token}`
      }
    })
    ...
    if (!response.ok) {
      throw new Error('CV nicht verfügbar')
    }
    ...
    let filename = `CV_${recommendation.value.candidateName.replace(/\s+/g, '_')}.pdf`
    ...
    alert('Fehler beim Download des CVs')
  } finally {
